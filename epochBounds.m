...
start_time = find(EXPT.time_stamps == EXPT.START_TIME);
%assert(start_time ~= 0, 'Start time not present in experiment time stamps');

end_time = find(EXPT.time_stamps == EXPT.END_TIME);
...
while((marker + epoch_length) <= EXPT.END_TIME)
    epoch_start = find(EXPT.time_stamps == marker);
    epoch_end = find(EXPT.time_stamps == (marker + epoch_length)) - 1;
